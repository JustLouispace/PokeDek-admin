...
const UpdateRequest = () => {
    ...
    const getSingleProduct = async () => {
        try {
            const { data } = await axios.get(`http://localhost:5000/api/Request/${params.slug}`);
            console.log(data);
            setName(data.name);
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            if (params.slug) {
                axios.post(`http://localhost:5000/api/PokemonCard`, formData, config)
                    .then((response) => {
                        // Assuming the POST request returns the created product's slug
                        const createdProductSlug = params.slug;
                        console.log(createdProductSlug);

                        return axios.delete(`http://localhost:5000/api/Request/${createdProductSlug}`);
                    })
                    .then(() => {
                        dispatch(resetState());
                        formik.resetForm();
    ...
    return (
        <div>
            <h1 className='mb-4'>Update And Add-Product</h1>
            <div>
